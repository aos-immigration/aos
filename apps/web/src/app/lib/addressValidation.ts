...
import { currentAddressSchema, previousAddressSchema } from "./schemas/addressSchema";
...
export function validateAllAddresses(
  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    const schema = addr.isCurrent ? currentAddressSchema : previousAddressSchema;

    // Use an empty string for missing required fields to let Zod handle it cleanly
    const safeParseResult = schema.safeParse({
      ...
      startMonth: addr.startMonth || "",
      startYear: addr.startYear || "",
      endMonth: addr.endMonth || "",
      endYear: addr.endYear || "",
    });

    if (!safeParseResult.success) {
      // Pick the first error message
      const firstError = safeParseResult.error.issues[0];
      errors[addr.id] = firstError?.message || "Invalid address";
      continue;
    }

    // Then check for overlaps with subsequent addresses
