...
export function validateAllAddresses(
  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    // Choose schema based on isCurrent
...
export function validateAddress(
  ...
  const schema = address.isCurrent ? currentAddressSchema : previousAddressSchema;
  const result = schema.safeParse(address);

  if (!result.success) {
    // Map Zod issues to ValidationErrors
    // We try to map path to keys, but path can be nested.
    // Here we just use the path[0] as key.
